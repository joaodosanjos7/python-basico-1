...
# Obtém o caminho absoluto para um recurso, funcionado tanto
...
# Decide onde o arquivo de banco de dados deve ficar: ao lado do .exe (quando empacotado),
# ou ao lado do script durante o desenvolvimento - assim os cadastros não
# se perdem a cada nova versão do programa.
def get_db_path():
...
# simples e sem dependências externas - mais seguro do que armazenar a 
...
class Database:
    ...
    def __init__(self, db_name=None):
       ...
       # cria as tabelas de usuários e credenciais, se não existirem
       def create_tables(self):
        ...
        # cria o usuário admin na primeira execução, se ele ainda não existir
        ...
        # Verifica se usuário e senha batem com hash salvo no banco
    ...
    # insere um novo usuario cadastrado
...
class TelaCadastro(ctk.CTk):
    ...
    def setup_ui(self):
        ...
        self.email_entry = ctk.CTkEntry(
            self.frame, placeholder_text="E-mail"
        )
        self.email_entry.pack(pady=5, padx=10, fill="x")
        self.telefone_entry = ctk.CTkEntry(self.frame, placeholder_text="Telefone")
        self.telefone_entry.pack(pady=5, padx=10, fill="x")self.telefone_entry.bind("<KeyRelease>", self.formatar_telefone)
...
self.listar_btn = ctk.CTkButton(
    self.btn_frame, text="Listar Cadastros", image=self.listar_icon,
...
def alternar_tema(self):
    ...
    def setup_ui(self):
        ...
        self.label = ctk.CTkLabel(
            self.frame, text="Lista de Usuários", font=("Roboto", 28)
        )
        ...
        self.voltar_btn = ctk.CTkButton(
            self.btn_frame, text="Voltar", image=self.voltar_icon, compound="left", command=self.voltar,
        )
...
def configurar_estilo_treeview(self):
    ...
    self.style.configure("Treeview.Heading", background="flat")
